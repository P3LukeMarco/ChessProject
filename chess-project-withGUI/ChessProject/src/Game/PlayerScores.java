...
public class PlayerScores {
    ...
    public void addPlayer(Player player, int score){
        ...
        int index = 0;
        if(set.containsKey(player.playerName.toUpperCase())) {
            ...
            if(option.nextLine().equals("yes")) {
                set.replace(player.playerName.toUpperCase(), score);
            }
        }
        else {
            set.put(player.playerName.toUpperCase(), score);
            System.out.println("player saved");
            try(FileWriter fw = new FileWriter("ExistingPlayers.txt", true);
                BufferedWriter bw = new BufferedWriter(fw);
                PrintWriter out = new PrintWriter(bw))
            {
                out.println(player.playerName.toUpperCase() + player.playerScore);
